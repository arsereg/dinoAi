...
void writeInfo() {
  ...
  if (showBestEachGen) { //if showing the best for each gen then write the applicable info
    text("Puntuación: " + genPlayerTemp.score, 30, height - 30);
    //text(, width/2-180, height-30);
    textAlign(RIGHT);
    text("Gen: " + (genPlayerTemp.gen +1), width -40, height-30);
    textSize(20);
    int x = 580;
    text("Distancia con el siguiente obstaculo", x, 18+44.44444);
    text("Altura del obstaculo", x, 18+2*44.44444);
    text("Ancho del obstaculo", x, 18+3*44.44444);
    text("Altura del ave", x, 18+4*44.44444);
    text("Velocidad", x, 18+5*44.44444);
    text("Posición del dinosaurio en Y", x, 18+6*44.44444);
    text("Espacio entre obstaculos", x, 18+7*44.44444);
    text("Bias", x, 18+8*44.44444);

    textAlign(LEFT);
    text("Salto pequeño", 1220, 118);
    text("Salto Grande", 1220, 218);
    text("Agacharse", 1220, 318);
  } else { //evolving normally 
    text("Score: " + floor(pop.populationLife/3.0), 30, height - 30);
    text("Dinosaurios vivos: " + pop.alive, 30, height - 70);
    //text(, width/2-180, height-30);
    textAlign(RIGHT);
